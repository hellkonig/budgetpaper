...
\begin{figure*}
\centerline{
  \psfig{figure=m500c_frac.eps,width=1.00\textwidth}
}
\caption{{\bf Upper panel shows baryon fraction comparing with 
   \citet{Voort16} (left) and \citet{Christensen16} (right), and lower
   panel shows stellar fraction comparing with \citet{Voort16} (left) 
   and \citet{Christensen16} (right).
   The blue points are from this work, the red pentagons are from
   \citet{Voort16} and the magenta pentagons are from 
   \citet{Christensen16}.}}
\label{fig:fire_com}
\end{figure*}
...
fraction.
Meanwhile, \citet{Voort16} have similar result produced by zoom-in 
cosmological hydrodynamic simulation suit FIRE which has 16 galaxies 
with similar halo mass range.
Fig.~\ref{fig:fire_com} compares baryon and stellar fraction with 
\citet{Voort16} and \citet{Christensen16}, we derive
new baryonic mass and stellar mass within R$_{500}$ and R$_{100}$
to make sure the ratios compared in each panel have same definitions with
ratios from other simulation suites. The baryon fraction in NIHAO simulations
have good agreement with both of the simulation suite in the whole mass range
where NIHAO simulations explored. The massive haloes with 
...
For stellar fraction, \citet{Voort16} shows a almost same result with
our work, even at low mass range, the two galaixes may suggest the 
large scatter as we produced. However, \citet{Christensen16} predicts
one magnitude higher stellar fraction than what we found at the halo 
mass range belowe $10^{11.5} \Msun$, which is expected by the
comparason with abundance-matching-derived fits shown in Fig. 3 in
their paper.}
