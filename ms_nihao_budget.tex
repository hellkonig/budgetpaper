...
{\bf Since the resolution is one key parameter in simulation, 
the convergence of the baryonic fraction predicted by NIHAO suite
is worth to estimate. Fig.~\ref{fig:budget_res} shows baryonic 
fraction color coded by number of total particles per halo.
In the plot, the number of total partilce per halo spans 2 magnitudes
and has no correlation with baryonic fraction. Therefore, we can conclude 
the resolution of NIHAO suite is proper to study the fractional 
baryon content.}
...
\begin{figure}
\centerline{
  \psfig{figure=budgetpro.eps,width=0.48\textwidth}
}
